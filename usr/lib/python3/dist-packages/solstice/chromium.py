...
def update_profile(iteminfo, profilename, profilepath, darkmode, nocache):
    ...
    result = chromi_set_sitepermissions(result, iteminfo["id"], iteminfo["website"], iteminfo["extraids"])
    ...
    chromi_add_startpage(iteminfo["id"], profileid, iteminfo["name"], iteminfo["website"])
    ...
    chromi_update_local_state(iteminfo["id"], profileid)

    #and finish off with this:
    chromi_finishing_touches(iteminfo["id"], profileid)


#Default Settings
def chromi_set_sitepermissions(preferencedict, itemid, ogwebsite, extraids):
    #dict, string, list
    ...
    for extraid in extraids:
        ...
        targetidinfo = getInfo(targetid, "", "") #no source is given in here, and Ice is 'source-agnostic'

        shortenedurl = get_shortened_url(targetidinfo["icewebsite"])
        try:
...
def chromi_set_bonuses(preferencedict, bonuses=[]):
    ...
    with open("/usr/share/solstice/chromiums/extras.json", 'r') as fp:
...
#Local State
def chromi_update_local_state(itemid, profileid):
    #string, string, bool
...
#Start Page
def chromi_add_startpage(itemid, profileid, name, website):
    #dict, string, string
...
#Finishing touches
def chromi_finishing_touches(itemid, profileid):
    #string, string
